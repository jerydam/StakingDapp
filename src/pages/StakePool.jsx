        ...
        <div className=" px-2 py-16 sm:px-0 overflow-clip">
            ...
            <div className='w-3/4 grid grid-cols-3 gap-4 m-auto'>
                <div className=' col-span-2'>
                    <Tab.Group >
                        <Tab.List className="flex space-x-1 bg-emerald-100/20 rounded-lg ring-1 ring-blue-500  p-1">
                            {categories.map((category) => (
                                <Tab
                                    key={category}
                                    className={({ selected }) =>
                                        classNames(
                                            'w-full py-2.5 text-sm font-medium leading-5 text-purple-700',
                                            'ring-white ring-opacity-60 ring-offset-2 focus:outline-none focus:ring-2',
                                            selected
                                                ? 'bg-blue-900 rounded-lg text-white shadow'
                                                : 'text-purple-600 hover:bg-white/[0.12] hover:text-blue-900'
                                        )
                                    }
                                >
                                    {category}
